...
export default function useWebRTC() {
    ...
    React.useEffect(() => {
        if (socket) {
            socket.on(SocketActions.ADD_PEER, async ({ peerId, createOffer, userId }) => {
                ...
                const name = users && users.length ? users.find(user => user.id === userId)?.firstName : "";
    ...
    const onEnd = () => {
        if (socket && callId && user) {
            ...
            const socketObject: { roomId: string; usersInCall?: IUser[]; } = {
                roomId: callId,
            };
            ...
            // Если статус звонка - Вызов, я инициатор и отменяю звонок - то необходимо отрисовать сообщение с отмененным звонком
