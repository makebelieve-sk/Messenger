...
export default memo(function Buttons({ status, settings, visibleVideo, onAccept, onToggle, onEnd }: IButtons) {
    // Завершение звонка
    const endCall = (title: string = "Завершить звонок") => <Tooltip title={title}>
        <Button color="error" variant="outlined" className="call-container__button" onClick={onEnd}>
        ...
        case CallStatus.NEW_CALL:
            return <>
                ...
                <Tooltip title="Принять">
                    <Button color="success" variant="outlined" className="call-container__button" onClick={onAccept}>
            ...
            return <>
                <Tooltip title={`${settings[SettingType.AUDIO] ? "Выключить" : "Включить"} микрофон`}>
                    <Button
                        ...
                        className="call-container__button"
