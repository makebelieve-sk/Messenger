...
export default class MessagesController extends EventEmitter {
	...
	private _init() {
		...
		this._socket.on(SocketActions.MESSAGE, ({ data, usersInChat }, callback) => {
			const validateData = validateHandleEvent(SocketActions.MESSAGE, {
				data,
				usersInChat,
			});
		...
		this._socket.on(SocketActions.DELETE_MESSAGE, ({ companionId, messageId }, callback) => {
			const validateData = validateHandleEvent(SocketActions.DELETE_MESSAGE, {
				companionId,
				messageId,
			});
		...
		this._socket.on(SocketActions.DELETE_CHAT, ({ chatId, companionId }, callback) => {
			const validateData = validateHandleEvent(SocketActions.DELETE_CHAT, {
				chatId,
				companionId,
			});
		...
		this._socket.on(SocketActions.EDIT_MESSAGE, ({ data, usersInChat }, callback) => {
			const validateData = validateHandleEvent(SocketActions.EDIT_MESSAGE, {
				data,
				usersInChat,
			});
		...
		this._socket.on(SocketActions.CHANGE_READ_STATUS, ({ isRead, messages }, callback) => {
			const validateData = validateHandleEvent(SocketActions.CHANGE_READ_STATUS, {
				isRead,
				messages,
			});
		...
		this._socket.on(SocketActions.NOTIFY_WRITE, ({ isWrite, chatId, usersInChat }, callback) => {
			const validateData = validateHandleEvent(SocketActions.NOTIFY_WRITE, {
				isWrite,
				chatId,
				usersInChat,
			});
			...
			if (validateData.success) {
				...
				this.emit(SocketEvents.NOTIFY_FEW_ANOTHER_USERS, usersInChat, SocketActions.NOTIFY_WRITE, data);
			}
