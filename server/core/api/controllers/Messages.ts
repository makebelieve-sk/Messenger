...
// import type { Literal, Where } from "sequelize/types/utils";
...
// 			res.json({
// 				success: true,
// 				unreadChatIds: unreadChatIds[0] && unreadChatIds[0][0] ? unreadChatIds[0][0].unReadChatsCount : 0,
// 			});
// 		} catch (error) {
// 			next(error);
// 		}
...
// 						chatObject.unReadMessageIds = unReadMessages[0].length ? unReadMessages[0].map(unReadMessageId => unReadMessageId.messageId) : [];
// 					} else {
...
// 								where: {
// 									id: {
// 										[Op.in]: filesInMessage.map(fileInMessage => fileInMessage.fileId),
// 									},
// 								},
// 								attributes: [ "id", "name" ],
// 								transaction,
...
// 			updatedChats.sort((a, b) => new Date(b.messageObject.createDate).getTime() - new Date(a.messageObject.createDate).getTime());
...
// 				this._database.sequelize.literal(
// 					"not exists (select 1 from [Deleted_messages] as [DeletedMessages] where [DeletedMessages].[message_id] = [Messages].id)",
// 				),
// 			];
...
// 			await this._database.models.readMessages.bulkCreate(readMessages, {
// 				transaction,
// 			});
...
// 				await this._database.models.filesInMessage.bulkCreate(filesInMessage, {
// 					transaction,
// 				});
// 			}
...
// 			await this._database.models.messages.update({ type, message }, { where: { id } });
...
// 				await this._database.models.chats.create({ id: chatId }, { transaction });

// 				await this._database.models.usersInChat.create({ id: uuid(), userId, chatId }, { transaction });

// 				await this._database.models.usersInChat.create({ id: uuid(), userId: friendId, chatId }, { transaction });
// 			}
...
// 				res.json({
// 					success: true,
// 					usersInChat: usersInChat.map(userInChat => userInChat.User),
// 				});
// 			} else {
